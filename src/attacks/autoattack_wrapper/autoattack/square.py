...
from src.attacks.autoattack_wrapper.autoattack.autopgd_base import L1_projection
...
class SquareAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        logits = self.predict(x.reshape(x.shape[0], *self._orig_shape))
    ...
    def perturb(self, x, y=None):
        ...
        if self.x_shape is None:
            self.x_shape = self._orig_shape
        x = x.reshape(x.shape[0], *self.x_shape)
        ...
        if y is None:
            if not self.targeted:
                with torch.no_grad():
                    output = self.predict(x.reshape(x.shape[0],
                                                    *self._orig_shape))
        ...
        if not self.targeted:
            acc = self.predict(x.reshape(x.shape[0],
                                         *self._orig_shape)).max(1)[1] == y
        else:
            acc = self.predict(x.reshape(x.shape[0],
                                         *self._orig_shape)).max(1)[1] != y
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                output_curr = self.predict(adv_curr.reshape(
                    adv_curr.shape[0], *self._orig_shape))
                if not self.targeted:
                ...
                adv = adv.reshape(adv.shape[0], *self._orig_shape)
